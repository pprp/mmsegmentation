...
from_path = '/HOME/scz0088/run/datasets/weights/model_r50_rf.pth.tar'
save_path = './work_dirs/resnet50_rf/model_r50_rf.pth.tar'
para_dict = torch.load(from_path)#['state_dict']
new_dict = OrderedDict()
print(para_dict.keys())
for key, value in para_dict.items():
    # print(key, key.replace('attention', 'receptive_field_attention'))
    new_dict[key.replace('receptive_field_attention', 'attention')] = value
torch.save(new_dict, save_path)